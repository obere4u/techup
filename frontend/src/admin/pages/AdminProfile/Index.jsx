...
export default function AdminProfile() {
  ...
  const [formData, setFormData] = useState({ currentAdmin });
  ...
  const handleUpdateClick = async (e) => {
    ...
    try {
      ...
      const adminUpdateResponse = await axios.put(
        TALENT_API_URL + `/update-admin/${currentAdminId}`,
      ...
      if (adminUpdateResponse.status === 200) {
        ...
        dispatch(updateSuccess(adminUpdateResponse.data));
        Swal.fire({
          icon: "success",
        ...
        const errorMsg = adminUpdateResponse.res?.data?.message;
  ...
  const handleCancelEditDetails = () => {
    // Reset the formData state to the currentAdmin data
