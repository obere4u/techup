...
// import { RxCaretDown, RxCalendar } from "react-icons/rx";
...
export default function TalentForm() {
  ...
  // const allStatesAndLGAs = nigerianStates.all(); // all states and LGAs
  // let errorMessage = ""; //error message

  //Checking for internet connectivity always
  // useEffect(() => {
  ...
  //       icon: "error",
  //       title: `${errorMessage}`,
  //       footer: "Please connect to a network",
  //     });
  //     return;
  //   }
  ...
  //   try {
  //     const response = await axios.post(`${TALENT_API_URL}`, data);
  ...
  //     <p className="text-center py-3 text-[1.15rem] font-semibold text-[#101010]">
  //       All fields marked <span className="text-[#FF5252]">*</span> are
  //       important for Talents
  //     </p>
  //     <form
  ...
  //                 <select
  //                   id="stateOfResidence"
  //                   className="relative w-full appearance-none z-[999] text-[#101010] text-[0.8rem] md:text-[1.15rem] bg-transparent tracking-wider cursor-pointer pl-2"
  ...
  //             disabled
  //             type="submit"
  //             className={`cursor-not-allowed bg-slate-300 font-bold py-2 px-4 text-[1.3rem] rounded-md text-white ${
  //               disableButton
  ...
  //check for access token first before navigating
  const handleNavigateToLoginPage = () => {
    access_token ? navigate("/talent-dashboard") : navigate("/talent-signin");
  };

  return (
    <ClientLayout>
  <div className="h-full sm:w-[95%] mx-auto flex flex-col space-y-4 items-center justify-center">
    
      <p className="sm:text-3xl font-semibold">
      Registration is closed for new entries.
      </p>
    
    <div className=" w-full sm:w-[95%] flex items-center justify-center space-x-4">
      <span className="sm:text-xl text-center">Registered before?</span>
      <div className="text-center flex items-center space-x-2 hover:space-x-4 border-b-2 border-blue-700 text-1xl sm:text-2xl text-blue-700 cursor-pointer hover:text-blue-800">
        <span onClick={handleNavigateToLoginPage}>Login here</span>{" "}
        <RiLogoutCircleLine />
      </div>
    </div>
  </div>
